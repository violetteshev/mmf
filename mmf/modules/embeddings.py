...
class BertVisioLinguisticKnwlgEmbeddings(BertEmbeddings):
    def __init__(self, config, *args, **kwargs):
        super().__init__(config)
        self.token_type_embeddings_visual = nn.Embedding(
            config.type_vocab_size, config.hidden_size
        ...
        self.projection = nn.Linear(config.visual_embedding_dim, config.hidden_size)
        self.use_knwlg = config.use_knwlg

        if self.use_knwlg:
            self.entity_combination = nn.Linear(config.hidden_size, config.hidden_size)
            self.entity_projection = nn.Linear(config.hidden_size, config.knwlg_size)
    ...
    def forward(
        ...
        visual_embedding = [batch_size, image_feature_length, image_feature_dim]
        image_text_alignment = [batch_size, image_feature_length, alignment_dim]
        entity_ids = [batch_size, sequence_length, entity_length]
        """
        ...
        # Compose txt entities
        composed_entities = None
        if self.use_knwlg:
            composed_entities = torch.zeros_like(words_embeddings)
            ...
            for i in range(batch_size):
                indices = entity_ids[i]
                for ii, idx in enumerate(indices):
                    c = self.entity_combination(torch.sum(words_embeddings[i, idx, :], dim=0))
                    #TODO: add smth like c = self.LayerNorm(c)
                    composed_entities[i, ii] = c
            
            # Project to knowledge size
            composed_entities = self.entity_projection(composed_entities)
        ...
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings, composed_entities
