...
class AdaptVisualBERTBase(BertPreTrainedModel):
    def __init__(
        self,
        config,
...
class AdaptVisualBERTForPretraining(nn.Module):
    def __init__(self, config):
        ...
        if self.bert_model_name is None:
            ...
            self.bert = AdaptVisualBERTBase.from_pretrained(
...
class AdaptVisualBERTForClassification(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.output_attentions = self.config.output_attentions
...
@registry.register_model("visual_bert_adapt")
class AdaptVisualBERT(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/visual_bert_adapt/pretrain.yaml"

    def build(self):
        if self.config.training_head_type == "pretraining":
            self.model = AdaptVisualBERTForPretraining(self.config)
        else:
            self.model = AdaptVisualBERTForClassification(self.config)
