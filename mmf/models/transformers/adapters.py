...
class BertAttention(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.output = BertSelfOutput(config, adapter_config)
...
class BertLayer(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.attention = BertAttention(config, adapter_config)
        self.is_decoder = config.is_decoder
        if self.is_decoder:
            self.crossattention = BertAttention(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config, adapter_config)
...
class BertEncoder(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.layer = nn.ModuleList([BertLayer(config, adapter_config) for _ in range(config.num_hidden_layers)])
...
class AdapterConfig(object):
    def __init__(self, hidden_act="gelu", adapter_size=64, adapter_initializer_range=0.0002):
...
class Adapter(nn.Module):
    def __init__(self, hidden_size, adapter_config):
        ...
        self.activation = ACT2FN[self.adapter_config.hidden_act] \
